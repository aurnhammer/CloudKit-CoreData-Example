...
public class Zip {
    ...
    public class func unzipFile(_ zipFilePath: URL, destination: URL, overwrite: Bool, password: String?, progress: ((_ progress: Double) -> ())?) throws {
        ...
        repeat {
			...
			guard pathString.utf8.count > 0 else {
            ...
            let directoryAttributes: [FileAttributeKey : Any]? = [FileAttributeKey(rawValue: FileAttributeKey.creationDate.rawValue) : creationDate, FileAttributeKey(rawValue: FileAttributeKey.modificationDate.rawValue) : creationDate]
            do {
                if isDirectory {
